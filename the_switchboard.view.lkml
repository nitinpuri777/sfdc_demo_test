...
view: opportunity_attributable_campaign_temp {
  derived_table: {
    sql: SELECT DISTINCT ocr.opportunity_id
...
view: company_campaign_temp {
  derived_table: {
    sql: SELECT DISTINCT MD5(REGEXP_REPLACE(TRIM(BOTH ' ' FROM REGEXP_REPLACE(LOWER(COALESCE(l.company, a.name)), '(\.|,|\s)(inc|ltd|llc|llp|incorporated)+[^a-zA-Z]*$', '')), '[[:punct:]]', '')) AS company_id
...
## This PDT was previously regenerated using SELECT COUNT(*) FROM public.lead
## We changed this to a calendered rebuild to avoid accidentally querying tables that are empty while ETL is still running
  ...
  dimension_group: campaign_touch {
    type: time
  ...
  set: detail {
    fields: [id, name, type, first_seen_date, attributable_campaign.grouping, attributable_campaign.name, salesrep.name]
  }
}
